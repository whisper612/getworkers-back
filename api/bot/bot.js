...
module.exports = function(app, bot, telegramObject, telegrafObject, pool) {
	...
	bot.hears('🗄️ Регистрация', (ctx) => {
		ctx.reply('Для  продолжения регистрации, пожалуйста, нажмите кнопку отправки номера телефона ☎️', Extra.markup((markup) => {
			markup.resize()
			.keyboard([
			...
			if (ctx.contact.phone_number !== undefined) {
				ctx.reply('Успешно! Для завершения регистрации, пожалуйста, укажите своё имя.', (ctx) => {
					bot.hears(ctx.text, (ctx) => {
						...
						return ctx.reply('Регистрация успешно завершена, если вы ошибилсь при написании имени, то сообщите об этом администратору.')
					})
				})
			}  else {
	...
	bot.hears('Для  продолжения регистрации, пожалуйста, нажмите кнопку отправки номера телефона.', (ctx) => {
